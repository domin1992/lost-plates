...
@section('subcontent')
    <h1 class="text-lg md:text-2xl font-display">{{ __('markersShow.' . $type) }} <a href="{{ $marker['plate']['link'] }}" class="text-purple-heart hover:text-cyan focus:text-cyan">{{ $marker['plate_number'] }}</a></h1>

    <div class="flex flex-col lg:flex-row lg:gap-4 mt-4">
        <div class="lg:basis-2/3">
            @if ($marker['additional_info'])
                <h2 class="md:text-xl font-display">
                    {{ __('markersShow.description') }}
                </h2>
            ...
            @if ($marker['phone_number'] || $marker['has_email'])
                <h2 class="md:text-xl font-display">
                    {{ __('markersShow.contactWithMarkerOwner') }}
                </h2>
